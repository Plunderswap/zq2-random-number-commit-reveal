...
contract DecodeRandomReveal is Script {
    ...
    function decodeAndDisplayRNG1(bytes memory logData, address walletAddress) internal {
        ...
        console2.log("Target Block:", targetBlock);
        console2.log("Target Block Hash:");
        console2.logBytes32(targetBlockHash);

        console2.log("\n=== Verification ===");
        if (walletAddress != address(0)) {
            uint256 calculatedRandom = verifyRandomCalculation(walletAddress, targetBlockHash);
            ...
            console2.log("Formula: keccak256(userAddress, targetBlockHash)");
        }

        // Display entropy analysis
        console2.log("\n=== Entropy Analysis ===");
        console2.log("User Address: HIGH (user-specific entropy)");
        console2.log("Target Block Hash:", targetBlockHash == bytes32(0) ? "ZERO (window expired)" : "NON-ZERO (good entropy)");
        console2.log("TOTAL: 2 entropy sources");
    }
    ...
    function decodeAndDisplayRNG2(bytes memory logData, address walletAddress) internal {
        ...
        console2.log("Expected 192 bytes for RNG2, got:", logData.length);
    }
}
